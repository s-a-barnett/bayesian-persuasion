...
```{r}
d_test <- read_csv("../stickgame/webppl-src/model-comparison/pp_map_beliefs.csv") %>%
  rename(genBelief = belief) %>%
  select(gameid, genBelief) %>%
  full_join(d_het_n24) %>%
  filter(agent1stick == 0.6) %>%
  filter(firstStick == 0.9)
```
...
d_pp <- read_csv("../stickgame/webppl-src/model-comparison/pp_mean_beliefs.csv") %>%
  rename(genBelief = belief) %>%
  select(gameid, genBelief) %>%
  full_join(d_het_n24) %>%
...
ggplot() +
  geom_point(data=d_bystick, aes(x=agent1stick, y=empirical_stat)) +
  geom_errorbar(data=d_bystick, aes(x=agent1stick, ymin=ci_lower, ymax=ci_upper), width=0) +
  geom_point(data=d_pp, aes(x=agent1stick, y=empirical_stat), colour="red") +
  geom_errorbar(data=d_pp, aes(x=agent1stick, ymin=ci_lower, ymax=ci_upper), width=1, colour="red") +
  geom_hline(yintercept=0.5, linetype='dotted') + 
  facet_grid( ~ firstStick) +
  ggthemes::theme_few() + 
  labs(x = 'agent 1 stick length', y = 'belief about mean') +
