...
cat(length(passed_manip_check)/2, 'participants passed bias manipulation check \n')
...
passed_speaker_criterion <- d %>%
  ...
  select(stimulus_contestant, button_pressed, gameid, rt) %>%
  spread(stimulus_contestant, button_pressed) %>%
  filter(first == 4 | second == 0 | third == 2 | fourth == 0) %>%
  pull(gameid)

cat(length(passed_speaker_criterion)/4, 'participants passed speaker exclusion \n')
cat(length(intersect(passed_rt_criterion, intersect(passed_speaker_criterion, passed_manip_check))), 'participants passed all criteria \n')
```
...
beliefChange.d %>%
  ...
  ggplot(aes(x = agent1stick, y = empirical_stat)) +
    ...
    labs(x = 'agent 2 stick length', y = 'absolute change in belief', 
         title = 'agent 1 stick = 0.6')

beliefChange.d %>%
  filter(agent2stick == 0.8) %>%
  group_by(agent1stick) %>%
  tidyboot::tidyboot_mean(diff) %>%
  ggplot(aes(x = agent1stick, y = empirical_stat)) +
    geom_point() +
...
beliefChange.d %>%
  ...
  ggplot(aes(x = agent2stick, y = empirical_stat)) +
    ...
    ggthemes::theme_few() +
    labs(x = 'agent 1 stick length', y = 'absolute change in belief')
