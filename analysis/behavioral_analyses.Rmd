...
ggplot() + 
  ...
  geom_errorbar(data=d_stick4, aes(x=agent1stick, ymin = ci_lower, ymax = ci_upper), width = 0) +
  geom_hline(yintercept = 50, linetype = 'dotted') +
  geom_line(data=(d_model %>% filter(judge == 'J1') %>% filter(nSticks == 3) %>% filter(agentBias == 5)), aes(x=agent1stick, y=percent_isLong_1)) + 
  ggthemes::theme_few() +
  labs(x = 'agent 1 stick length', y = 'belief about mean') +
  xlim(0.5,1) +
  ylim(0,100) + 
  ggtitle('Sceptics, J1 judge, nSticks = 3')
```
...
ggplot() + 
  ...
  geom_errorbar(data=d_stick3, aes(x=agent1stick, ymin = ci_lower, ymax = ci_upper), width = 0) +
  geom_hline(yintercept = 50, linetype = 'dotted') +
  geom_line(data=(d_model %>% filter(judge == 'J0') %>% filter(nSticks == 5)), aes(x=agent1stick, y=percent_isLong_1)) + 
  ggthemes::theme_few() +
  labs(x = 'agent 1 stick length', y = 'belief about mean') +
  xlim(0.5,1) +
  ylim(0,100) + 
  ggtitle('Naïfs, J0 judge, nSticks = 5')
```
