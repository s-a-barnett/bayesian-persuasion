...
```{r}
d_raw <- read_csv(here('data/replication.csv')) %>%
...
cat('of', d_raw %>% pull(gameid) %>% unique() %>% length(), 'participants recruited\n')
cat(length(complete_games), 'participants completed both isLong trials \n')
```

### Look more carefully at attention checks
...
internally_consistent <- d_raw %>%
  ...
  select(gameid, firstContestant, stimulus_type, agent1stick, agent2stick, response) %>%
  spread(stimulus_type, response) %>%
  filter(!is.na(genModel_fifth)) %>%
  mutate(genModel = (10*agent1stick + 10*agent2stick + `genModel_third` + `genModel_fourth` + `genModel_fifth`)/5,
         passedBias = ifelse(firstContestant == 'long', bias_first >= 45 & bias_second <= 55, bias_first <= 45 & bias_second >= 55),
         ...
         allPassed = passedBias & genModelConsistent)

passedBiasCheck <- internally_consistent %>% filter(passedBias) %>% pull(gameid)
passed2afc <- internally_consistent %>% filter(`2afcConsistent`) %>% pull(gameid)
passedGenModel <- internally_consistent %>% filter(genModelConsistent) %>% pull(gameid)
passedBoth <- intersect(passedBiasCheck, passedGenModel)
...
d %>%
  ...
  mutate(first = ifelse(first <= 2, '<=7', first + 5)) %>%
  mutate(first = paste0('showed ', first, '" stick first')) %>%
...
internally_consistent %>%
  group_by(group = ifelse(speakerTrial_first < 3, '<= 2', speakerTrial_first)) %>%
  summarize(`% bias check` = mean(passedBias, na.rm = T), 
            `n` = length(passedBias),
...
## Look at other direction (i.e. speaker behavior conditioned on passing or not passing)

```{r}
cat(length(passedBiasCheck), 'participants passed bias manipulation check \n')

internally_consistent %>%
  mutate(speakerGroup = ifelse(speakerTrial_first < 3, '<= 2', speakerTrial_first)) %>%
  group_by(allPassed, speakerGroup) %>%
  tally() %>%
  filter(!is.na(allPassed)) %>%
  group_by(allPassed) %>%
  mutate(p = n / sum(n), n = sum(n)) %>%
  spread(speakerGroup,p)
...
internally_consistent %>%
  ...
  filter(!is.na(allPassed)) %>%
  group_by(allPassed) %>%
  spread(speakerGroup,n) %>%
  ungroup() %>%
  select(-allPassed) %>%
  chisq.test()

chisq.test(matrix(c(45, 107, 27, 138, 69, 389), ncol = 3))
```

## Scatter plots of continuous agreement

```{r}
ggplot(internally_consistent, aes(x = isLong_second, y = meanSlider)) +
    geom_point() +
    ylim(0,100) +
    xlim(0,100) +
    facet_wrap(~ speakerTrial_first) +
    geom_smooth(method = 'lm') +
    theme(aspect.ratio = 1)

ggplot(internally_consistent, aes(x = genModel * 10, y = meanSlider)) +
    geom_point(alpha = 0.5) +
    geom_abline(intercept = 0, slope = 1) +
    ylim(0,100) +
    xlim(0,100) +
    facet_wrap(~ ifelse(speakerTrial_first < 3, '<= 7', speakerTrial_first + 5)) +
    geom_smooth(method = 'lm') +
    labs(x = "generative estimate", y = "slider estimate") +
    theme_few() +
    theme(aspect.ratio = 1) 

internally_consistent %>%
  group_by(ifelse(speakerTrial_first < 2, 2, speakerTrial_first)) %>% 
  summarize(m = cor(genModel, meanSlider, method = 'spearman'))
ggsave('attentionCheckScatter.pdf', units = 'in', height = 4, width = 8)
```

## Scatter plots of 2AFC

```{r}
d %>%
  filter(gameid %in% complete_games) %>%
  left_join(speaker_criterion) %>%
  unite(stimulus_type, stimulus_type, stimulus_contestant, na.rm = T) %>%
  filter(stimulus_type %in% c('2afc', 'meanSlider', 'isLong_first', 'isLong_second')) %>%
  group_by(gameid, stimulus_type) %>%
  mutate(response = ifelse(is.na(response), button_pressed, response)) %>%
  select(-chosenStick, -responses, -button_pressed, -rt) %>%
  arrange(gameid) %>%
  pivot_wider(names_from = stimulus_type, values_from = response) %>%
  mutate(first = ifelse(first <= 2, '<=7', paste0(first + 5)),
         `2afc` = ifelse(`2afc` == 1, 'longer', 'shorter')) %>%
  filter(!is.na(`2afc`)) %>%
  ggplot(aes(x = as.factor(`2afc`), y = meanSlider, group = as.factor(`2afc`))) +
    geom_jitter(alpha = 0.25, width = 0.25, height = 0, data = . %>% filter(`2afc` == 'shorter' & meanSlider <= 50), color = 'blue') +
    geom_jitter(alpha = 0.25, width = 0.25, height = 0, data = . %>% filter(`2afc` == 'longer' & meanSlider >= 50), color = 'red') +
    geom_jitter(width = 0.25, height = 0, data = . %>% filter(`2afc` == 'shorter' & meanSlider > 55), color = 'black') +
    geom_jitter(width = 0.25, height = 0, data = . %>% filter(`2afc` == 'longer' & meanSlider < 45), color = 'black') +
    facet_wrap(~ first) +
    labs(x = '2AFC response', y = 'mean estimate') +
    ylim(0, 100) +
    theme_few()
```
