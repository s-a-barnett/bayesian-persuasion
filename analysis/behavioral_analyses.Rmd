...
d_raw %>%
  ...
  group_by(firstContestant, sequence) %>%
  tally() %>%
  arrange(firstContestant,-n)
```
...
fig2.d %>%
  ggplot(aes(x = observation, y = response)) + #color = firstContestant
    ...
    ggthemes::theme_few() +
    labs(x = 'observation (stick length)', y = 'belief about mean') +
    facet_wrap(~ first) +
...
ggsave('speaker_dependence_option_rawdata.pdf', units = 'in', height = 4, width = 6)
```
...
m2 <- d %>%
  ...
  mutate(observation = round(abs(.5 - first_observation), 2) - .25,
         ...
         first = paste0('expected ', first, ' evidence first')) %>%
  select(response, observation, first, firstContestant) %>%
  lm(response ~ observation * first, 
     contrasts = list(first = contr.sum(2)), 
     data = .) 
